...
@inject('videoStore')
@observer
class TrtcRoom extends React.Component {
    ...
    startScreenSharing(fn = () => { }) {
        return new Promise((resolve, reject) => {
            ...
            this.shareClient.join({ roomId: this.roomid }).then(() => {
                this.shareStream = TRTC.createStream({ audio: false, screen: true });
                this.shareStream.initialize().then(() => {
                    this.shareClient.publish(this.shareStream).then(() => {
                        resolve();
                    }).catch((err) => {
    ...
    closeScreenSharing() {
        if (!this.shareClient && !this.shareStream) return;
        this.shareClient.unpublish(this.shareStream).then(() => {
            // 取消发布本地流成功
    ...
    onRemoteStreamRemove() {
        this.client.on('stream-removed', () => {
            ...
            const parentVideo = document.querySelector('#remoteVideo');
            const remoteVideoDom = document.querySelector(`#remoteStream-${this.id}`);
    ...
    getRemoteVideoStats() {
        this.remoteTimer = setInterval(() => {
            this.client.getRemoteVideoStats().then(stats => {
                const strengthReady = document.querySelector('.strength-ready');
                const wifi2 = [1, 2];
                const wifi3 = [1, 2, 3];
                for (let customerCode in stats) {
                    ...
                    if(this.customerCode == customerCode && bytesReceived >= 70) {
                        for(let i = 1; i <= 4; i++) {
                            document.querySelector(`.wifi-${i}`).setAttribute('style', 'color: green');
                        }
                    ...
                    if(this.customerCode == customerCode && bytesReceived >= 30 && bytesReceived <= 69) {
                        ...
                        for(let i = 0; i < wifi3.length; i++) {
                            document.querySelector(`.wifi-${wifi3[i]}`).setAttribute('style', 'color: #e9a028');
                        }
                        document.querySelector('.wifi-4').setAttribute('style', 'color: #a79e91');
                    ...
                    if(this.customerCode == customerCode && bytesReceived > 0 && bytesReceived <= 29) {
                        ...
                        for(let i = 0; i < wifi2.length; i++) {
                            document.querySelector(`.wifi-${wifi2[i]}`).setAttribute('style', 'color: red');
                        }
                        document.querySelector('.wifi-4').setAttribute('style', 'color: #a79e91');
                        document.querySelector('.wifi-3').setAttribute('style', 'color: #a79e91');
                    ...
                    if(this.customerCode == customerCode && bytesReceived == 0) {
                        ...
                        document.querySelector('.wifi-4').setAttribute('style', 'color: #a79e91');
                        document.querySelector('.wifi-3').setAttribute('style', 'color: #a79e91');
                        document.querySelector('.wifi-2').setAttribute('style', 'color: #a79e91');
                        document.querySelector('.wifi-1').setAttribute('style', 'color: #a79e91');
    ...
    render() {
        ...
        const { videoStore: { customerInfo, insurantName, requestId, flowId, isVideo, isAudio, isShowModal, isShowWaiting, countDown, isScreenSharing }, videoStore } = this.props;
        return (
            <section>
                {
                    <div
